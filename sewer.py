...
class PredictionModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=1024, aspect_ratios=[[1]], scales=[1], parent=None, index=0):
        ...
        self.mask_dim    = cfg.mask_dim # Defined by Sewer
        self.num_priors  = sum(len(x)*len(scales) for x in aspect_ratios)
        self.parent      = [parent] # Don't include this in the state dict
        self.index       = index
        self.num_heads   = cfg.num_heads # Defined by Sewer
...
class Sewer(nn.Module):
    ...
    def __init__(self):
        ...
        # Compute mask_dim here and add it back to the config. Make sure Sewer's constructor is called early!
        if cfg.mask_type == mask_type.direct:
